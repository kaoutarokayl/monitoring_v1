...
Nom du groupe : Support (Ce nom doit correspondre exactement à ce qui est attendu par le backend).

Conserver "Globale" et "Sécurité" cochés, puis valider par OK.

(Optionnel) Répéter l'opération pour créer le groupe Superviseur.
...
Taper Support dans la zone de saisie.
...
public class AtmController : ControllerBase
{
    ...
    [HttpPost("clients")]
    [Authorize(Policy = "RequireWrite")]   // ← Support uniquement
...
|---|---|
| `isSupport()` | `true` si l'utilisateur a le rôle `Support` |
| `isReadOnly()` | `true` si `Superviseur` SANS `Support` |
...
```html
<!-- Masque le bouton pour Superviseur -->
<button *appHasRole="['Support']" (click)="delete()">
  🗑 Supprimer
</button>

<!-- Visible pour les deux rôles -->
<div *appHasRole="['Superviseur', 'Support']">
...
{ 
  ...
  canActivate: [roleGuard],          // ← bloque Superviseur
  data: { roles: ['Support'] }
}
```

**Comportement :**
- `Superviseur` tente `/admin/atms/create` → redirigé vers `/access-denied`
- `Support` → accès autorisé ✅

---

## PARTIE 4 — Matrice des droits

| Fonctionnalité | `Superviseur` | `Support` |
|---|:---:|:---:|
...
<button *appHasRole="['Support']">Action sensible</button>
